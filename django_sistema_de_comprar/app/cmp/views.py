...
def proveedor_inactivar(request, id):
    template_name = 'cmp/deactivate.html'
    context = {}
...
def compras(request, compra_id=None):
    ...
    if request.method == 'POST':
        ...
        if det:
            det.save()
            sub_total = ComprasDet.objects.filter(compra=compra_id)\
                .aggregate(Sum('sub_total'))
            descuento = ComprasDet.objects.filter(compra=compra_id)\
                .aggregate(Sum('descuento'))
...
class CompraDetDelete(SinPrivilegios, generic.DeleteView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            'cmp:compras_edit', kwargs={'compra_id': compra_id})
